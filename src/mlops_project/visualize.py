...
def visualize(cfg: DictConfig) -> None:
    ...
    class_wise_errors = cm.sum(axis=1) - cm.diagonal()  # Errors per class
    ...
    fig, axes = plt.subplots(len(struggling_classes), 5, figsize=(15, len(struggling_classes) * 4))  # 5 images per class
    for i, cls in enumerate(struggling_classes):
        ...
        for j, (img, lbl, pred) in enumerate(cls_images[:5]):  # Show up to 5 images
            ax = axes[i, j] if len(struggling_classes) > 1 else axes[j]
            img = inv_transform(img)  # Inverse normalize and convert to PIL image
            ax.imshow(img)
            ax.axis('off')
